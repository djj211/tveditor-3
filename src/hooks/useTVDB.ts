...
export const useTVDB = () => {
    ...
    const searchOne = async(show: string): Promise<TVDBItem> => {
    ...
    const search = async (show: string, limit?: number): Promise<TVDBItem[]> => {
        ...
        const allDataPromises = resp.data.map(async(t) => {
            ...
            const latest = seriesEps.data.episodes.reduce((acc, curr) => {
