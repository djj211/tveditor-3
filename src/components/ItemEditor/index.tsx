...
const ItemEditor = ({ item, open, handleClose }: Props ) => {
    ...
    const onSubmit = async(e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (season && episode && !editLoading) {
            await editShow(item.flexget.id, item.flexget.name, season, episode);
          ...
          <Dialog open={open} onClose={onClose} aria-labelledby="form-dialog-title">
            ...
            <DialogContent>
                ...
                <InfoItem 
                  ...
                  value={formatEpisodeText(item.flexget.latest_entity?.season, item.flexget.latest_entity?.number, item.flexget.latest_entity?.first_seen)} />
                <InfoItem 
                  label="Last Aired" 
                  value={formatEpisodeText(item.tvdb.latestEpisode?.season, item.tvdb.latestEpisode?.number, item.tvdb.latestEpisode?.airDate)} />
                <InfoItem 
                  label="Status" 
                  value={item.tvdb.status}
                  color={item.tvdb.status !== "Continuing" ? "red" : ""}
                  />
