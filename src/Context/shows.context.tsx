...
const initialState: ShowsContextType = {
  ...
  addShow: async (_name: string, _season: number, _episode: number, _tvdbId: string) => {
    return;
  },
  editShow: async (_showId: number, _name: string, _season: number, _episode: number, _tvdbId: string) => {
    return;
  },
  deleteShow: async (_showId: number, _name: string) => {
    return;
  },
...
const ShowsContextProvider = ({ children }: { children?: React.ReactNode }) => {
  ...
  const theRefresh = React.useMemo(
    () => (usingProxy ? proxyRefresh : doRefresh),
    [usingProxy, proxyRefresh, doRefresh],
  );
  ...
  const addShow = async (name: string, season: number, episode: number, tvdbId: string) => {
    setEditLoading(true);
    try {
      await theAdd(name, season, episode, tvdbId);
      await theRefresh();
    } catch (ex) {
      throw ex;
    } finally {
  ...
  const editShow = async (showId: number, name: string, season: number, episode: number, tvdbId: string) => {
    setEditLoading(true);
    await theEdit(showId, name, season, episode, tvdbId);
    await theRefresh();
    setEditLoading(false);
  };
